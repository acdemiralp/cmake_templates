...
```text
Create a new [executable|header-only library|library] [with vcpkg] called [project_name] from acdemiralp/cmake_templates and apply the setup steps from the repository readme.
```
Alternatively:
- Copy the desired template directory and rename it to your project name.
- Rename the `./include/PROJECT_NAME_HERE` to your project name.
...
## Notes
- Templates follow modern CMake best practices and the [VCPKG CMake Style Guide](https://learn.microsoft.com/en-us/vcpkg/contributing/cmake-guidelines), with the following exceptions: GLOB is used, alignment is used, variable names are uppercase.
