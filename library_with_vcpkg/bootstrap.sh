...
"$SCRIPT_DIR/build/vcpkg/vcpkg" --vcpkg-root "$SCRIPT_DIR/build/vcpkg" install --x-manifest-root "$SCRIPT_DIR" --x-install-root "$SCRIPT_DIR/build/vcpkg_installed"

CMAKE=$("$SCRIPT_DIR/build/vcpkg/vcpkg" --vcpkg-root "$SCRIPT_DIR/build/vcpkg" fetch cmake | tail -1)
